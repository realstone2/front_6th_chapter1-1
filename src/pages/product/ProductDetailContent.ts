...
export class ProductDetailContent extends Component<{}, { cartCount: number; product: ProductDetail | null }> {
  constructor() {
    super({}, { cartCount: 1, product: null });
  }
  ...
  componentDidMount() {
    // store 구독해서 product 상태 갱신
    productDetailStore.subscribe(() => {
      this.state = {
        ...this.state,
        product: productDetailStore.value.data,
      };
    });
    // 최초 상태 세팅
    this.state = {
    ...
    EventDelegator.getInstance().register("click", "quantity-decrease", () => {
      this.state = {
        ...this.state,
        cartCount: Math.max(1, this.state.cartCount - 1),
      };
    });
    EventDelegator.getInstance().register("click", "quantity-increase", () => {
      const { product, cartCount } = this.state;
      this.state = {
        ...this.state,
        cartCount: Math.min(cartCount + 1, product?.stock ?? 1),
      };
    });
    EventDelegator.getInstance().register("change", "quantity-input", (e) => {
      ...
      this.state = {
        ...this.state,
        cartCount: Math.max(1, Math.min(isNaN(value) ? 1 : value, product?.stock ?? 1)),
      };
    });
  }
  ...
  componentWillUnmount() {
    ...
    EventDelegator.getInstance().unregister("change", "quantity-input");
  }
}
